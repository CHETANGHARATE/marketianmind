...
<div class="space-y-6 max-w-5xl">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-6">
        <div>
            <div class="flex items-center gap-2.5">
                ...
                <span class="text-xs text-slate-500">Edit Template</span>
            </div>
            <h1 class="mt-2 text-2xl sm:text-3xl font-bold tracking-tight text-white">
                Edit Template: {{ $template->name }}
            </h1>
            <p class="mt-1 text-sm text-slate-400">
                Update template content, subject, or placeholder tags.
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
            <form action="{{ route('admin.marketing-templates.update', $template) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 space-y-4 shadow-xs">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="slug" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">
                                Unique Slug <span class="text-rose-400">*</span>
                            </label>
                    ...
                    <div>
                        ...
                        <textarea id="body_html" name="body_html" rows="14" required
                                  class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs text-white font-mono placeholder-slate-500 focus:outline-none focus:border-indigo-500 leading-relaxed">{{ old('body_html', $template->body_html) }}</textarea>
                        @error('body_html')
                            <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
