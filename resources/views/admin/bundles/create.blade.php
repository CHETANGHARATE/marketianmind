...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-white">Create Course Bundle</h1>
            <p class="mt-1 text-sm text-slate-400">Assemble multiple courses into a high-converting package.</p>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('title') }}"
                           required
                           placeholder="e.g., Growth Marketing Master Suite 2026"
                ...
                <div class="sm:col-span-2">
                    <label for="slug" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
                        URL Slug <span class="text-slate-500 font-normal lowercase">(optional, auto-generated from title)</span>
                    </label>
                    <input type="text"
                           name="slug"
                           id="slug"
                           value="{{ old('slug') }}"
                           placeholder="growth-marketing-master-suite"
                           class="w-full rounded-lg border border-slate-700 bg-slate-800/80 px-3.5 py-2 text-sm text-white placeholder-slate-400 focus:border-amber-500 focus:outline-hidden focus:ring-1 focus:ring-amber-500">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="short_description"
                              ...
                              rows="2"
                              placeholder="Brief value proposition shown on cards and summaries..."
                              class="w-full rounded-lg border border-slate-700 bg-slate-800/80 px-3.5 py-2 text-sm text-white placeholder-slate-400 focus:border-amber-500 focus:outline-hidden focus:ring-1 focus:ring-amber-500">{{ old('short_description') }}</textarea>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description"
                              id="description"
                              rows="5"
                              placeholder="Detailed breakdown of what this bundle delivers..."
                              class="w-full rounded-lg border border-slate-700 bg-slate-800/80 px-3.5 py-2 text-sm text-white placeholder-slate-400 focus:border-amber-500 focus:outline-hidden focus:ring-1 focus:ring-amber-500">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach(\App\Enums\BundleStatus::cases() as $statusCase)
                            <option value="{{ $statusCase->value }}" {{ old('status', 'draft') === $statusCase->value ? 'selected' : '' }}>
                                {{ $statusCase->label() }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900/60 p-6 space-y-4">
            <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">Select all courses that will be granted upon purchasing this package.</p>
                </div>
                <span class="text-xs font-semibold text-slate-400">Select at least 1 course</span>
            </div>

            @if($courses->isEmpty())
                <p class="text-sm text-amber-400">No courses are currently available to add to bundles.</p>
            @else
