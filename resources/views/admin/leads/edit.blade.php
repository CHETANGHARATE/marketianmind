...
<div class="max-w-4xl space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.leads.show', $lead) }}" class="text-xs font-semibold text-slate-400 hover:text-white transition">
            &larr; Back to Lead Details
        </a>
    ...
    <div>
        ...
        <p class="text-xs text-slate-400 mt-1">
            Update prospect information, pipeline status, assignment, or follow-up schedule.
        </p>
    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text"
                           name="country"
                           id="country"
                           value="{{ old('country', $lead->country ?? 'India') }}"
                           class="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:border-amber-500 focus:outline-hidden">
                </div>
            </div>
        </div>

        <!-- Pipeline Attributes & Educational Interest Card -->
        <div class="rounded-xl border border-slate-800 bg-slate-900/70 p-6 space-y-5">
            <h2 class="text-sm font-bold uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-3">
                2. Pipeline Status &amp; Interest
            </h2>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="status" class="block text-xs font-bold uppercase tracking-wider text-slate-300">
                        Pipeline Status <span class="text-amber-400">*</span>
                    </label>
                    <select name="status"
                            id="status"
                            required
                ...
                <div>
                    ...
                    <select name="priority"
                        ...
                        @foreach(\App\Enums\LeadPriority::cases() as $pr)
                            <option value="{{ $pr->value }}" {{ old('priority', $lead->priority->value) === $pr->value ? 'selected' : '' }}>
                                {{ $pr->label() }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="assigned_to"
                        ...
                        @foreach($assignees as $staff)
                            <option value="{{ $staff->id }}" {{ old('assigned_to', $lead->assigned_to) == $staff->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="bundle_id"
                        ...
                        @foreach($bundles as $bundle)
                            <option value="{{ $bundle->id }}" {{ old('bundle_id', $lead->bundle_id) == $bundle->id ? 'selected' : '' }}>
                                {{ $bundle->title }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           id="follow_up_status"
                           value="{{ old('follow_up_status', $lead->follow_up_status) }}"
                           placeholder="e.g. pending, call_scheduled, completed"
                           class="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3.5 py-2.5 text-xs text-white focus:border-amber-500 focus:outline-hidden">
                </div>
            </div>
        </div>
