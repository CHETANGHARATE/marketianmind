    ...
    <div class="flex items-center justify-between border-b border-slate-800 pb-6">
        <div>
            <div class="flex items-center gap-2.5">
                ...
                <span class="text-xs text-slate-400">Edit Offer</span>
            </div>
            <h1 class="mt-2 text-2xl font-bold tracking-tight text-white">
                Edit Promotional Offer: {{ $offer->name }}
            </h1>
            <p class="mt-1 text-sm text-slate-400">
                Update promotional offer parameters, schedules, product assignments, and discount values.
            </p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="discount_type"
                        ...
                        <option value="percentage" {{ old('discount_type', $offer->discount_type->value) === 'percentage' ? 'selected' : '' }}>Percentage (%) Off</option>
                        <option value="fixed" {{ old('discount_type', $offer->discount_type->value) === 'fixed' ? 'selected' : '' }}>Fixed Amount (₹) Off</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description"
                              id="description"
                              rows="3"
                              class="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-950/60 p-3.5 text-xs text-white placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">{{ old('description', $offer->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number"
                           name="priority"
                           id="priority"
                           value="{{ old('priority', $offer->priority) }}"
                           min="0"
                           max="9999"
                           class="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-950/60 px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                </div>
                ...
                <div>
                    <label for="usage_limit" class="block text-xs font-semibold text-slate-300">
                        Total Usage / Redemption Cap <span class="text-slate-500">(Current: {{ $offer->times_used }})</span>
                    </label>
                    <input type="number"
                           name="usage_limit"
                           id="usage_limit"
            ...
            <div class="pt-4 border-t border-slate-800/80 space-y-3">
                <label class="relative flex items-start gap-3 cursor-pointer">
                    ...
                    <input type="checkbox"
                           name="is_active"
                           value="1"
                           {{ old('is_active', $offer->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
                ...
                <label class="relative flex items-start gap-3 cursor-pointer">
                    ...
                    <input type="checkbox"
                           name="allow_coupons"
                           value="1"
                           {{ old('allow_coupons', $offer->allow_coupons ? '1' : '0') == '1' ? 'checked' : '' }}
                           class="h-4 w-4 rounded border-slate-700 bg-slate-950 text-amber-500 focus:ring-amber-400 mt-0.5">
                    <div>
                        ...
                        @foreach($courses as $course)
                            <label class="flex items-start gap-2.5 p-2.5 rounded-xl border border-slate-800 bg-slate-950/40 hover:border-slate-700 transition cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       {{ in_array($course->id, old('courses', $selectedCourseIds)) ? 'checked' : '' }}>
                                <div class="text-xs">
                        ...
                        @foreach($bundles as $bundle)
                            <label class="flex items-start gap-2.5 p-2.5 rounded-xl border border-slate-800 bg-slate-950/40 hover:border-slate-700 transition cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       {{ in_array($bundle->id, old('bundles', $selectedBundleIds)) ? 'checked' : '' }}>
                                <div class="text-xs">
